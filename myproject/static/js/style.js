...
async function submitForm() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            appendMessage("bot", "left", result.test_cases, result.screen_name, randomId);
            saveResponse(screen_name, requirement, result.test_cases, );
        } else {
            if (response.status === 500) {
                console.error("❌ Lỗi 500: Internal Server Error");
                appendMessage("bot", "left", "Hệ thống gặp lỗi nội bộ. Vui lòng thử lại sau.", "System", randomId);
            } else {
                console.error(`❌ Lỗi ${response.status}: ${response.statusText}`);
                appendMessage("bot", "left", `Lỗi ${response.status}: ${response.statusText}`, "System", randomId);
            }
        }
    } catch (error) {
                    ...
                    <div class="msg-bubble">
                        ...
                        <div class="msg-text">
                            <table class="table table-bordered" style="width: 1600px;">
                                ...
                                <tbody id="testcase-body-${chat.id}">
                                </tbody>
                            </table>
                        </div>
...
async function appendMessage(name, side, text, screen_name, id) {
    const responsesContainer = document.getElementById("responses");
